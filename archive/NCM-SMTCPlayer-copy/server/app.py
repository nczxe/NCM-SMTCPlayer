...
<head>
    ...
    <style>
        * {
        ...
        .logged-in-bar .logout-btn:active {
            color: #e74c3c;
        }
...
<body>
    ...
    <div class="container">
        <div class="tab-nav">
            <button class="tab-btn active" data-tab="nowplaying">正在播放</button>
        ...
        <div class="tab-content" id="tab-search">
            <div class="search-bar">
                <input type="text" class="search-input" id="searchInput" placeholder="搜索歌曲..." autocomplete="off">
        ...
        <div class="tab-content" id="tab-playlist">
            <div id="playlistLoginSection">
                <p style="font-size:12px;color:rgba(255,255,255,0.45);margin-bottom:10px;line-height:1.6;">
                    1. 浏览器打开 music.163.com 并登录<br>
                    2. F12 → Application → Cookies → 找到 <b>MUSIC_U</b><br>
                    3. 复制其<b>值</b>粘贴到下方
                </p>
                <input type="text" id="loginCookie" placeholder="粘贴 MUSIC_U cookie 值" style="width:100%;padding:12px 16px;margin-bottom:10px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.1);border-radius:12px;color:#fff;font-size:15px;outline:none;">
                <button class="btn-primary" onclick="doLogin('cookie')">Cookie 登录</button>
                <div id="loginError" style="color:#e74c3c;font-size:13px;text-align:center;margin-top:8px;display:none;"></div>
            </div>
            <div id="playlistLoggedIn" class="hidden">
                <div class="logged-in-bar">
                    <span class="user-info" id="playlistUserInfo"></span>
            ...
            <div id="playlistDetail" class="hidden">
                <div class="playlist-header" onclick="backToPlaylists()">← 返回歌单列表</div>
                <div class="song-list" id="playlistSongList"></div>
            </div>
        </div>
    ...
    <script>
        ...
        async function searchSongs(keywords) {
            ...
            try {
                ...
                container.innerHTML = '<div class="empty-state"><div class="icon">😞</div><div>搜索失败，请稍后重试</div></div>';
            }
        }
        ...
        function renderSearchResults(songs, total) {
            const container = document.getElementById('searchResults');
            if (!songs.length) {
                container.innerHTML = '<div class="empty-state"><div class="icon">🔍</div><div>未找到相关歌曲</div></div>';
                return;
            }
            let html = '';
            songs.forEach((s, i) => {
                html += `<div class="song-item" onclick="playNcmSong(${s.id}, '${escapeHtml(s.name)}')">
                    ...
                    <div class="song-item-cover" style="background-image:url('${s.cover || ''}')">${s.cover ? '' : '🎵'}</div>
        ...
        async function fetchPlaylists() {
            ...
            try {
                ...
                grid.innerHTML = '<div class="empty-state"><div class="icon">😞</div><div>加载失败</div></div>';
            }
        }
        ...
        function renderPlaylists(playlists) {
            const grid = document.getElementById('playlistGrid');
            if (!playlists.length) {
                grid.innerHTML = '<div class="empty-state"><div class="icon">📋</div><div>暂无歌单</div></div>';
                return;
            }
            let html = '';
            playlists.forEach(pl => {
                html += `<div class="playlist-card" onclick="fetchPlaylistDetail(${pl.id}, '${escapeHtml(pl.name)}')">
                    <div class="playlist-card-cover" style="background-image:url('${pl.cover || ''}')">${pl.cover ? '' : '🎵'}</div>
        ...
        async function fetchPlaylistDetail(playlistId, playlistName) {
            ...
            try {
                ...
                listEl.innerHTML = '<div class="empty-state"><div class="icon">😞</div><div>加载失败</div></div>';
            }
        }
        ...
        function renderPlaylistSongs(tracks) {
            const listEl = document.getElementById('playlistSongList');
            if (!tracks.length) {
                listEl.innerHTML = '<div class="empty-state"><div class="icon">📋</div><div>暂无歌曲</div></div>';
                return;
            }
            let html = '';
            tracks.forEach((t, i) => {
                html += `<div class="song-item" onclick="playNcmSong(${t.id}, '${escapeHtml(t.name)}')">
                    <div class="song-item-index">${i + 1}</div>
                    <div class="song-item-cover" style="background-image:url('${t.cover || ''}')">${t.cover ? '' : '🎵'}</div>
                    <div class="song-item-info">
